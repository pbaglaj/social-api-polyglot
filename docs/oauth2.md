...
1. Użytkownik klika „Zaloguj" → `signinRedirect()` → przeglądarka leci na Keycloak (`:8090`).
...
4. UI ukrywa akcje wg ról (`useWhoami` → `GET /api/admin/me`): panel Admina i tworzenie
   tagów tylko dla `Admin`, `attach` dla `Admin`/`Moderator`.
5. Panel **„Google Calendar"** woła `GET /api/google/calendar` (Faza 4) — działa po
...
- **SPA:** `http://localhost:5173` → zaloguj (`user/user` lub `admin/admin`) → CRUD; panel
  Admina widoczny tylko dla `admin`.
...
- **Google Calendar:** zaloguj się w SPA **przez Google** (ekran Keycloak → „Google") →
  panel „Google Calendar" → „Pobierz nadchodzące wydarzenia" → lista wydarzeń (`200`).
