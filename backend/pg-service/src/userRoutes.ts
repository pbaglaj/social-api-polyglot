...
router.post('/:id/follow', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
...
router.delete('/:id/follow', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const followeeParam = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  if (!followeeParam) {
    res.status(400).json({ error: 'Validation Error', code: 'INVALID_FOLLOWEE_ID', details: 'No followee ID provided.' });
    return;
