...
export function Stats({ userId }: { userId: number }) {
  ...
  return (
    <section className="panel" id="stats">
      <h2>Statystyki (pg native)</h2>
      <div className="row">
        <input placeholder="userId" value={statsUserId} onChange={(e) => setStatsUserId(e.target.value)} />
        <button onClick={() => void userStats()}>Statystyki użytkownika</button>
      </div>
      <div className="row">
        <label>
          limit:
        ...
        <input placeholder="since (ISO, opcj.)" value={since} onChange={(e) => setSince(e.target.value)} />
        <button onClick={() => void topPosts()}>Top posty</button>
      </div>
